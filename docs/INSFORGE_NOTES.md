...
There is also a **daily token cap per model** that the headers do not advertise. After roughly six rounds in a day, `gpt-oss-120b` answered normal-sized requests with 429 and `Retry-After: 257`, then `2463` seconds, while a 3-token request still succeeded. Each model has its own bucket, so the referee now takes a comma-separated chain (`HEALER_MODEL=openai/gpt-oss-120b,qwen/qwen3.8-27b,openai/gpt-oss-20b`), skips a model for as long as its Retry-After says, and records the model that answered on every `healer_thought` event so results stay attributable. A Retry-After above 90 s ends the round with an error instead of blocking the public queue, and self-play is cut to every 12 hours so the budget goes to visitors. One public C01 round also sat 12 minutes on a single request before per-request timeouts (90 s) and an in-call wall-clock deadline were added.

Per-request caps differ by model: `qwen/qwen3.8-27b` rejects any request over **7,000 input tokens per minute** with a 413, while the gpt-oss models allow 8,000. With the full SKILL.md the fixed part of every healer request (system prompt ≈ 5.2k tokens + 17 tool schemas ≈ 1.3k) left no room for history. On non-Anthropic providers H2 therefore carries a compact cut of the vendored skill: the primitives table and the six app-level recipes (≈ 1.7k tokens; fixed part ≈ 3.9k), selected by `HEALER_SKILL=full|core|compact`. A 413 falls through to the next model in the chain, and the chain is ordered 120b → 20b → qwen so the 8k-cap models are tried first.
